...
def test_change_mem(base_url, token, create_a_post):
    headers = {
        'Authorization': token,
        'Content-Type': 'application/json'
    }
    ...
    assert response["tags"] == ["cooking", "cats", "chicken", "fun"]
...
def test_delete_mem(base_url, token, create_a_post):
    headers = {
        'Authorization': token
    }
    requests.request(
